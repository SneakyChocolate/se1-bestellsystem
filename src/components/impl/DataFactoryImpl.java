...
class DataFactoryImpl implements DataFactory, Validator {
     ...
     * @param taxRate rate according to {@link Pricing.TAXRate} ({@code TAXRate.Regular} is default)
     * @return {@link Article} object created from valid arguments or empty
     */
    public Optional<Article> createArticle(
        String description,
        long unitPrice,
        Pricing.PricingCategory pricingCategory,
        Pricing.TAXRate... taxRate
    ) {
        Pricing.TAXRate tax_rate = taxRate.length > 0? taxRate[0] : Pricing.TAXRate.Regular;
    ...
    {
        return createOrder(Pricing.PricingCategory.BasePricing, customer, orderConsumer);
    }
     ...
     */
    public Optional<Order> createOrder(
        Pricing.PricingCategory category,
        Optional<Customer> customer,
        Consumer<Order> orderConsumer
    ) {
        if(category==null)
