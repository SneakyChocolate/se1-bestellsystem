...
public class Application_E12 implements Runtime.Runnable {
	...
	public void run(Properties properties, String[] args) {
		...
		List.of(
				...
				dataFactory.createCustomer("Mandy Mondschein", "locomandy<>gmx.de")
					.map(c -> c.addContact("+49 030-3956256")),
				dataFactory.createCustomer("", "nobody@gmx.de")).stream()
		...
		List.of(
				...
				dataFactory.createArticle("Fahrradkarte", 695, PricingCategory.BasePricing, TAXRate.Reduced)).stream()
			.flatMap(Optional::stream)
	...
	public StringBuilder printOrders(Collection<Order> orders) {
		...
		final var tf = new TableFormatter(
			...
			.line() // table header
			.row("Bestell-ID", String.format("Bestellungen%s", currencyLabel), "MwSt", "*", "Preis", "MwSt",
				"Gesamt")
			.line();
		//
		...
		//
		tf.row(null, null, null, null, "Gesamt:", fmtPrice(compound[1], Pricing.Currency.Euro),
			fmtPrice(compound[0], Pricing.Currency.Euro));
	...
	TableFormatter printOrder(Order order, long orderValue, long orderVAT, TableFormatter tf) {
		...
		for (int i = 0; it.hasNext(); i++) {
			...
			String itemDescr = String.format(" - %dx %s%s",
				unitsOrdered, descr,
				unitsOrdered > 1 ? String.format(", %dx %s", unitsOrdered, fmtPrice(unitPrice, currency))
					: String.format(""));
			...
			tf.row("", itemDescr, fmtPrice(vat, currency), reducedTaxMarker, fmtPrice(value, currency), totals[0],
				totals[1]);
		}
		return tf;
	}
	...
	private void buildOrders(Map<Long, Order> collector) {
		...
		final OrderBuilder orderBuilder = dataFactory.createOrderBuilder(
			pricing,
			this::findCustomerBySpec,
			this::findArticleBySpec);

		List.of(
				// Eric's 1st order
				orderBuilder.buildOrder("Eric", buildState -> buildState
					.item(4, "Teller") // 4 Teller, 4x 6.49 €
				...
				orderBuilder.buildOrder("Anne", buildState -> buildState
					.item(2, "Teller") // 2 Teller, 2x 6.49 €
				...
				// Nadine-Ulla's order
				orderBuilder.buildOrder("Nadine-Ulla", buildState -> buildState
					.item(12, "Teller") // 12 Teller, 12x 6.49 €
					.item(1, "Buch 'Java'") // 1 Buch "Java", 1x 49.90 €
					.item(1, "SKU-425378") // 1 Buch "UML", 1x 79.95 €
				),

				// Khaled Saad's order
				orderBuilder.buildOrder("Khaled Saad", buildState -> buildState
					.item(1, "Buch 'Java'") // 1 Buch "Java", 1x 49.90 €
				...
				// Eric's 3rd order
				orderBuilder.buildOrder("Eric", buildState -> buildState
					.item(1, "Fahrradhelm") // 1 Fahrradhelm, 1x 169.00 €
					.item(1, "Fahrradkarte") // 1 Fahrradkarte, 1x 6.95 €
				),

				// Eric's 4th order
				orderBuilder.buildOrder("Eric", buildState -> buildState
					.item(3, "Tasse") // 3 Tassen, 3x 2.99 €
